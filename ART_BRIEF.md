...
3. Generate `three_quarter.png`, `side.png`, `back.png`, each using `front.png` as reference plus an explicit angle instruction (e.g. _"same character, same outfit and colours, now shown from directly behind, full profile silhouette"_).
...
**Generation order:** lock Drip Pistol at 0° first (style template for every gun), generate its other 7 angles from that reference, author its `anchor.json`, then repeat per-gun for the remaining 5 (each gets its own 0° lock, then 7 angles, using Drip Pistol's 0° as a _style_ reference so outline/lighting stay consistent across guns even though the gun shapes differ).
