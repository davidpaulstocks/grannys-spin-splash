...
**Currently working on:** Real granny/gun/world/title art landed overnight (2026-09-11) — Sprint 3 is now done. `Granny.ts` and `Gun.ts` were rewritten from placeholders to render the delivered sprites: Granny swaps `back`/`back_firing` textures on fire state, Gun snaps to the nearest of 8 pre-rotated angle sprites from live aim direction and exposes the nozzle position (from anchor.json) as the water-particle spawn origin, replacing the old generic gun-origin point. SplashScene's three carousels now show real art via `Carousel.ts`'s existing `swatchTextureKey` support, and the title area is the illustrated flourish + wordmark lockup instead of plain text. A real bug was caught in live-browser verification and fixed: the gun's hand-grip offset was computed against Granny's vertical centre instead of her feet, putting the gun up near her hair — re-derived from her feet position using a pixel-measured hand-height fraction (~58% up) from the actual art. Moved on into Sprint 7 QA: all 3 Poki canonical sizes (640×360/836×470/1031×580) live-verified across multiple granny/gun/world combos; incognito/storage-denied mode live-verified via a patched-`localStorage` test against the real `SaveManager` module (confirmed the sole call site app-wide); a 5-simulated-second continuous-fire stress test found no errors and a comfortable frame-time margin (desktop proxy, real mobile profiling still open). Walked the full CLAUDE.md §12 pre-submission checklist against the real code (story 7.5) and found + fixed a real gap it surfaced: `BootScene.ts` had been an empty stub since Sprint 0 (deliberately deferred — "nothing to preload yet" — back when the whole game was procedural placeholders), but now that ~5MB of real sprite art exists, SplashScene/GameScene's own `preload()` calls meant the very first paint of the night's new art would've streamed in with nothing shown while it loaded. Implemented `BootScene.ts` properly: preloads every launch sprite up front behind a simple on-brand loading bar, fires `gameLoadingFinished()` once, then hands off — Play now transitions instantly with zero loading gap. Also caught arrow-key capture only covered LEFT/RIGHT, not UP/DOWN, against §12 rule 3 — fixed. Sprint 4's 12-stem orchestra remains blocked on the audio source decision (§9.3); thumbnails (5.7) remain the one outstanding must-fix item, deferred for remaining-effort reasons, not art; cross-browser/real-device testing (7.2) still needs actual devices. Full pipeline (tsc/lint/test/build/budget) green — 6.29 MB / 8 MB.
