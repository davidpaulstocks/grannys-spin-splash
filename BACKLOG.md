...
Later the same session, Sprint 4's orchestra was built (synthesised placeholder voices per §9.3 option 4 — see CLAUDE.md's decisions log), and thumbnails/onboarding shipped (see git history for that wave).

Then a live playtest surfaced a cluster of real regressions and gaps, fixed in one pass — a comprehensive spec audit (10 parallel agents auditing every CLAUDE.md section + the prototype against actual source) ran alongside, confirming several of them independently and surfacing others fixed the same session:

- **The splash screen's world-switch arrows were genuinely broken, twice over.** First bug: `LoadoutStage`'s external arrows floated far enough outside its own card that they overlapped the neighbouring Granny/Gun selectors — whichever was added to the scene last painted over and ate clicks on the other's nearer arrow. Fixed by insetting every selector's arrows within its own footprint. Second bug, caught live testing the fix: the new inset `CompactSelector` added its arrows *before* its own opaque pill background, so they existed and worked but were invisible — the user's exact follow-up report, "I don't see the buttons to change granny or gun." Both components now add their arrows last, so they win the paint order and Phaser's topOnly input.
- **Granny/Gun selectors became real visual thumbnails again**, on direct request ("let players scroll through a carousel of thumbnails... name just under, beautiful composite in centre reflecting current choices") — new `ThumbnailSelector.ts` shows the actual granny/gun art with the name in its own row below the thumbnail+arrows row, so a long name ("The Squirt Sister") never shares horizontal space with an arrow the way the old text-only `CompactSelector` did (which is why long names used to render clipped). `CompactSelector.ts` deleted, fully superseded.
- **A live, exciting star tally during play** (`ScoreCounter.ts`) — direct feedback that not seeing currency accumulate live was a letdown. Counts up with a gold pop on every increase, superseding story 2.5's original "hide score until game over."
- **A distinct orchestra theme per world** — Garden (bird chirps, C major, 92bpm), Workshop (hammer clangs + a steel-drum voice, D minor, 100bpm), Kitchen (glassy kalimba + stovetop sizzle, F major, 98bpm), Funfair (oom-pah calliope, G major, 108bpm), Disco (real four-on-the-floor kick + syncopated bass + wah guitar, 120bpm) — `src/audio/themes/`, one theme-agnostic mixing graph so switching worlds needs no rebuild.
- **Two critical, player-facing bugs the audit caught:** mobile move buttons were also firing the water cannon and could trigger a false two-finger pause (Phaser's scene-wide pointer events fire for every touch regardless of what it hit) — fixed with a pointer-id exclusion set in `InputManager`, ordered so a button's own release can't be mistaken for a genuine lift and cancel an unrelated held fire touch. And any rejected `PokiSDK` promise (ad blocker, network failure) could permanently brick the PLAY button and leave audio muted forever — fixed with try/catch + a `finally` in `AdManager.ts`.
- **Smaller but real gaps the audit found and this pass closed:** the 60%/80% mini-frenzy thresholds were emitted but never listened for (now a toast + soft chime); Frenzy's own bonus stars had no floating-text feedback; the mid-run refill ad let the round timer keep advancing underneath it (now pauses the scene for the ad's duration); the two reward-button pairs violated §11.3's own "standard button must be larger-or-equal" and "must show a video icon" rules (fixed with a new `drawPlayIcon` custom glyph + explicit width floors); `GunDef.pump` was dead data describing a click-to-pump mechanic that was never built and doesn't match the actual (correctly prototype-extracted) flat-2-second-refill behaviour, so it was removed rather than left misleading.
- **Direct requests folded in alongside:** a desktop-only controls legend on the splash (`ControlsLegend.ts`); chunkier, higher-contrast mobile move buttons with a pressed state, sized for a young child's thumb; occasional "Nice!"/"Let's go!" encouragement toasts with a deliberately quiet SFX chime that doesn't compete with the orchestra; `BUTTON_ART_BRIEF.md` — Nano Banana prompts for the user to generate real move-button art, matching the existing delivery convention.

Verified with a real autonomous bot playtest, not just unit tests: three full 30-second rounds (Garden/squirter, Funfair/soaker3000, Disco/inferno) run end-to-end with a "commit to the weakest spinner until FULL" strategy, zero JS errors, 11–17 of each world's spinners individually reaching FULL, screenshots confirming the live score tally and wall art read correctly mid-play. None of the three reached full Splash Frenzy — worth noting honestly rather than hiding, though CLAUDE.md's own target is "~50% of runs," not every run, and the bot never used power-ups, which real players will.

All of the above is live-verified (see individual commit messages for exact test steps) and the full pipeline is green. Cross-browser/real-device testing (7.2) still needs actual devices; studio name (7.0) needs the user; the 4.6 "does it feel like ASMR" human playtest is still outstanding. Everything else is unblocked and this session kept moving through it autonomously per explicit standing instruction.
...
- ⚠️ Also over the 300-line cap: `spinnerRenderers.ts` (520, one bespoke draw function per world × type — a natural place to split by world if picked up), `SplashScene.ts` (385, grew with the new `ThumbnailSelector` wiring), `gun.data.ts`'s comment-heavy header aside. None flagged as urgent; the audit's minor findings list (2026-09-12) has the fuller list of §7.3-style nitpicks (magic numbers off the 8px grid, a few raw hex literals, missing JSDoc on some public methods) not acted on this session — real but low-value relative to what was.
- ⚠️ **Zero automated bot run reached full Splash Frenzy** in a 3-round live playtest (2026-09-12: Garden/squirter, Funfair/soaker3000, Disco/inferno — see this file's own log above), though 11–17 of each world's spinners individually reached FULL. CLAUDE.md's own target is "~50% of normal runs," not every run, and the bot used no power-ups (a real player will) and a simple "commit to the weakest spinner" strategy rather than an optimal one — so this isn't proof of a balance problem, but it's a data point worth a real human playtest confirming before submission, not just re-asserting the target is met.
...
- [x] **8.4** Game-over payoff screen — `GRANNY_LEVELS` was extracted from the prototype in Sprint 0 and then consumed by nothing, so the ladder the entire score economy climbs was invisible and the end of every run was a bare number on a cream background. Now: the score ticks up with a sound rather than appearing, the rank lands after a beat with a bounce in its own colour plus its flavour line, and the vault total is shown so the stars feel banked. Watching the double-score ad re-runs the whole beat on the doubled number, since the rank can genuinely change — which is the reason to have watched it.
  - Icons are deliberately not drawn: `GrannyLevel.iconKey` points into an icon sprite sheet that doesn't exist (`ICON_KEYS` is still `{}`), and a missing-texture box would read worse than the label alone. Wiring them is a follow-up if the icon sheet ever gets built.
...
- [x] **8.9** The water tank is now a signature feature, not a progress bar (direct user request: "bubbles, change colour as it depletes... make that super exciting"). `WaterBar.ts` rebuilt as a live body of liquid — a sloshing meniscus on the leading edge, 22 rising wobbling bubbles that surge while firing and boil while pumping, a gloss band and shaded floor for depth, and a colour that warms as it drains. Bar height 32 → 48 so the detail survives the scale-down to 640×360. All procedural Graphics: zero asset-budget cost. **Two real bugs found by measuring rendered pixels rather than eyeballing screenshots:** (1) the first colour ramp interpolated Water Blue straight to Heat Orange, which are near-complementary, so it passed through desaturated mud — measured #C19877 at 55% tank, a tan bar that vanished against the cream background. Now routed Blue → Sunny Gold → Heat Orange, holding pure blue above 60% so the shift is decisive; verified at the pixel level (100%/70% = #4DB3E5, 28% = #FFCE3F, 16% = #FFCC4C). (2) the low-water throb *darkened* the fill (0.76–1.0×), reading as dull olive exactly when it should feel urgent — now brightens (1.0–1.18×) so a low tank glows.
- [x] **8.8** Granny and gun were rendering far too small (direct user feedback). Root cause was not the sprite size but occlusion: at 160px tall sitting at y624 her feet landed at 704, which is 56px *below* the water bar's top edge, so a third of her was hidden behind the HUD and she only ever read as ~104px. Now 216px tall with her feet at 644, just above the bar — **2.08× her previous visible size** — with `GUN_LENGTH_PX` scaled by the same 1.35×. This deliberately exceeds §5.8's "~12% of vertical real estate" (she is now 30%); that figure was written before any art existed and reads far too small for an illustrated character. Confirmed at 640×360, 1031×580 and 375×812 portrait.
- [x] **8.7** **Splash Frenzy was mathematically unreachable — the game's central promise (§15) did not work.** A clean bot round with the starter pistol reached only 4 of Garden's 12 spinners at FULL and never triggered Frenzy, against story 2.8's "~50% of runs". Two independent causes, both found by diffing against the playtested prototype rather than re-tuning against ourselves:
  - **Coordinate-space regression.** The prototype runs on 960×540; this game runs 1280×720 (4/3 larger), but `WATER_PARTICLE_SPEED` (650), `WATER_PARTICLE_GRAVITY` (90) and granny's move speed (320) were copied across verbatim. Water therefore crossed a 33% wider wall at the prototype's speed while every spinner decay rate stayed byte-identical — silently making the game harder than the version a 6-year-old playtested. All three now scale by 4/3 (867/120/427), restoring the original geometry; particle lifetime derives from speed so it followed automatically.
  - **Delivery rate.** The prototype fires a particle every 28ms and each hit adds the *spinner's own* `power` (pinwheel 28, fan 32) — ~1000 units/s against a Garden wall decaying at 108/s total, nearly 10× headroom. Ours delivered 136/s theoretical, of which only ~44/s actually landed once pipeline overflow is counted (~0.6s of water is always in flight, so every target switch dumps charge onto an already-full spinner). Gun powers retuned to 409 → 700 units/s across the ladder.
  - **Verified with a control:** the identical in-flight-aware bot scores 0 Frenzy / 4-of-12 at the old power and 2 Frenzy / 12-of-12 at the new one, so the ladder — not the bot strategy — is what unlocked the mechanic. The top end was deliberately compressed to a 1.7× spread rather than the 2.5× straight scaling suggested: at 846/s a mid-tier gun hit Frenzy 8× in a single Disco round, and a permanently-full wall flatlines §9.1.1's per-spinner audio build, which is the headline feature.
  - ⚠️ **Measurement note worth keeping:** every earlier reading in this area was invalid. The Browser pane's throttled rAF injects frames with ~1s deltas between manual stepping chunks, which decays the entire wall and burns the round clock — a round "ran" 1300 frames but 29s of game time. `game.loop.stop()` before manual stepping is mandatory; with it, 300 frames advanced exactly 5000ms.
  - ⚠️ **Open, not fixed:** world difficulty ordering does not match the decay data. Disco (2,000★, meant to be the endgame) is *easier* to hold than Garden, because its roster is dominated by low-decay spinners (disco 2, propeller 3) — once filled it holds itself, whereas Garden's four fans (decay 15) fall out of FULL in 1.33s each. Inherited from the prototype's own numbers, so left alone rather than redesigned late; worth a deliberate pass.
- [x] **8.6** Hand-painted move-button art delivered by the user and wired in (see [BUTTON_ART_BRIEF.md](BUTTON_ART_BRIEF.md)) — the two edge move buttons were the last piece of UI still drawn as procedural `Phaser.Graphics`, correct on palette but visibly not sharing the watercolour language of everything around them. Four sprites now (rest + pressed, per direction), `MobileMoveButtons.ts` swapped off Graphics onto `Image`s. Three things worth knowing: (1) the art arrived as checkerboard-flattened JPEGs with no alpha, so transparency was reconstructed via threshold + 3px erode + premultiplied-alpha resampling — verified zero grey fringe, edges average the Ink outline colour; (2) each PNG's artwork sits in a 244px box inside a 256px canvas, so the sprite is drawn at `BUTTON_SIZE × 256/244` to keep the *visible* button exactly the 116 logical px the touch-target sizing was tuned against, rather than silently rendering ~5% smaller; (3) preloading is gated on touch via a shared `usesTouchMoveButtons()` predicate that `BootScene` and `MobileMoveButtons` both use — desktop never downloads the ~90 KB, and one predicate means the load gate and the build gate can't drift apart into missing-texture boxes. Verified live at 375×812 with touch emulated: all four textures load, rest/pressed swap correctly on press, `getMoveDir()` reads -1 while held, **pressing a move button still does not fire the cannon** (the exclusion fix from earlier the same day survives the sprite swap), release resets cleanly, no console errors — and re-verified on desktop that nothing loads, nothing is built, and no `__MISSING` texture is used.
- [x] **8.5** Extended 8.1's onboarding with the "mini scripted moments" the user explicitly asked for on top of §6.5's spec ("add a tutorial for first-time players... gives them a mini scripted moments, so they know what to do and how to play and the purpose of the game") — 8.1's ghost finger taught the *gesture* but never said what it was for. Two short held captions (`TutorialCaption.ts`, `TutorialSequence.ts`), first-ever run only, each tied to something that just happened rather than dumped up front: "Squirt the spinners to make them spin faster!" at round start alongside the existing ghost finger, and "Get every spinner to FULL for SPLASH FRENZY!" the moment their first spinner ever reaches FULL — the purpose-of-the-game beat, gated to fire once per round so a returning player who somehow triggers the callback path twice never sees it stack. §6.5 still rules out a tutorial *screen*; this is text layered over live gameplay, not a gate the player has to clear. Kept out of `GameScene.ts` itself beyond two call sites and a nullable field, matching this session's own lesson about a 711-line file: the sequencing and rendering both live in the new `ui/` files. Verified live: caption text/positioning/timing confirmed on a cleared save, the purpose caption confirmed by invoking it directly against the running scene (clicking through to a real FULL transition in this environment's browser automation doesn't reliably sustain "held to fire" long enough — a known input-timing limitation of the tooling, not the game), the once-per-round gate confirmed to no-op on a second call, and a `hasPlayed: true` save confirmed to show neither caption nor the ghost finger at all.
...
- **PLAY and the rewarded-ad button had a 0.15px gap — they touched.** A child aiming at PLAY and landing one pixel low got a full-screen ad instead of a game, on every returning visit. Re-spaced to 604/684, and the ad button is the one button in the game that opts out of the expanded hit zone (`expandHitArea: false`) precisely so it can never steal a tap meant for PLAY.
